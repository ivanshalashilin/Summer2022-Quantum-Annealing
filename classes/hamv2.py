...
class ParentHam:
    ...
    def find_flip_locations_1(self, coeffs, threshhold=0.05):
        ...
        for t in range(times):
            ...
            for i in range(prob_states):
                ...
                if flip == 0:
                    if (
                        abs(current_coeffs[i]) > threshhold
                        and abs(new_coeffs[i]) > threshhold
                    ):
                        all_flipped = False
            if all_flipped:
                flip_locations[t] = 1
    ...
    def sta_plot_evec(self, evals, evecs, n, m, plot=False, savefig=False, name=None):
        ...
        if plot == True:
            ...
            ax.set_title(
                "Components for n = %s = %.1f Eigenvalue" % (str(n), evals[-1][n])
            )
            ax.set_xlabel("$s$")
    ...
    def dyn_plot_cont(self, evecs, states, n=1, plot=False, savefig=False, name=None):
        ...
        for j in range(n):
            ...
            if plot == True:
                if j == 0 or j == 1:
                    ax.plot(
                        self._tsteps, overlap_squared, label=label, zorder=100, lw=4
                    )
                else:
                    # make higher energies gray
    ...
    def _label_states(self):
        ...
        for v in range(1, len(self._s_values)):
            ...
            for state in state_list:
                new_state_list.extend(
                    [
                        state + ";  " + str(self._s_values[v]) + "," + str(m)
                        for m in self._m_lists[v]
                    ]
                )
    ...
    def dyn_plot_cont(self, evecs, states, n=1, plot=False, savefig=False, name=None):
        ...
        for j in range(n):
            ...
            if plot == True:
                if j == 0 or j == 1:
                    ax.plot(
                        self._tsteps, overlap_squared, label=label, zorder=100, lw=3
                    )
                else:
                    # make higher energies gray
                    ax.plot(self._tsteps, overlap_squared, color="gray", lw=2)
